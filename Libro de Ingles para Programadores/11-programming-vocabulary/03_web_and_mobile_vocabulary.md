...
| **GraphQL** | /ɡræf-kju-ɛl/ "graf-kiu-el" | Lenguaje de consultas para APIs |
...
- "Create a reusable **widget** for the product card." (Crea un widget reutilizable para la tarjeta de producto.)
- "This **component** accepts three **props**." (Este componente acepta tres props.)
- "The **state** updates when the user clicks the button." (El estado se actualiza cuando el usuario hace clic en el boton.)
- "Use a **hook** to manage the form state." (Usa un hook para manejar el estado del formulario.)
...
| **Unmount** | /ʌnˈmaʊnt/ "an-MAUNT" | Remover un componente del arbol de UI |

- "The component **renders** a list of items." (El componente renderiza una lista de elementos.)
- "Fetch data when the widget **mounts**." (Obtiene datos cuando el widget se monta.)
- "Clean up the listener when the component **unmounts**." (Limpia el listener cuando el componente se desmonta.)
...
- "Add a **breakpoint** at 768 pixels." (Agrega un breakpoint en 768 pixeles.)
- "The **layout** uses a grid system." (El layout usa un sistema de grilla.)
...
| Ingles | Espaniol |
|--------|----------|
| "The API returns a 404 error." | "La API devuelve un error 404." |
| "Add a loading state to the component." | "Agrega un estado de carga al componente." |
| "The middleware rejects unauthorized requests." | "El middleware rechaza peticiones no autorizadas." |
| "Deploy the latest build to staging." | "Despliega el ultimo build a staging." |
| "The layout breaks on small screens." | "El layout se rompe en pantallas pequenias." |
| "Pass the callback as a prop." | "Pasa el callback como prop." |

---

## Resumen

El vocabulario de desarrollo web y movil se organiza en:

- **Arquitectura**: frontend, backend, fullstack, API, REST, GraphQL
- **Servidor**: endpoint, route, middleware, controller, service, repository
- **UI**: widget, component, state, props, hook, lifecycle, render, mount, unmount
- **Diseno**: responsive, viewport, breakpoint, layout, navigation
- **Infraestructura**: deployment, hosting, domain, SSL, CDN
